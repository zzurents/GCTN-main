...
class Dataset(object):
    ...
    def _load_data(self, config):
        format_dict = {"UIRTC": ["user", "item", "rating", "time","category"],
                       "UIRT": ["user", "item", "rating", "time"],
        ...
        if self._check_saved_data(splitter, ori_prefix, saved_prefix):
            ...
            by_time = config["by_time"] if file_format in {"UIRTC","UIRT", "UIT"} else False
        ...
        self.itemtrain_matrix = csr_matrix((train_ratings, (train_data["item"], train_data["user"])),
        ...
        if file_format in {"UIRT", "UIT"}:
            self.time_matrix = csr_matrix((train_data["time"], (train_data["user"], train_data["item"])),
                                          shape=(self.num_users, self.num_items))
            self.time_test_matrix = csr_matrix((test_data["time"], (test_data["user"], test_data["item"])),
                                          shape=(self.num_users, self.num_items))
        if file_format in {"UIRTC"}:
            self.time_matrix = csr_matrix((train_data["time"], (train_data["user"], train_data["item"])),
                                          shape=(self.num_users, self.num_items))
            self.timeitem_matrix = csr_matrix((train_data["time"], (train_data["item"], train_data["user"])),
                                          shape=(self.num_items, self.num_users))
            self.time_test_matrix = csr_matrix((test_data["time"], (test_data["user"], test_data["item"])),
                                               shape=(self.num_users, self.num_items))
            a = train_data["category"]
            b = train_data["item"]
            self.cat_matrix = csr_matrix((train_data["category"], (train_data["user"], train_data["item"])),
                                         shape=(self.num_users, self.num_items))
            self.itemcat_matrix = csr_matrix((train_data["category"], (train_data["item"], train_data["user"])),
                                         shape=(self.num_items, self.num_users))
            self.itemcat_matrixtwo = csr_matrix((test_data["category"], (test_data["item"], test_data["user"])),
                                             shape=(self.num_items, self.num_users))
    ...
    def get_train_interactionssecond(self):
        dok_matrix = self.cat_matrix.todok()
        users_list, items_list, category_list = [], [], []
        for (user, item), value in dok_matrix.items():
            users_list.append(user)
            items_list.append(item)
            category_list.append(value)

        return users_list, items_list, category_list
